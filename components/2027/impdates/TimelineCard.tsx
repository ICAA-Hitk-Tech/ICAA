  ...
  return (
    <article className="p-6 border border-ink bg-surface/40 shadow-[4px_4px_0px_0px_var(--color-ink)] hover:bg-surface hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_0px_var(--color-ink)] transition-all duration-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      <div className="shrink-0 flex flex-col items-start md:items-end gap-1.5">
        {item.oldDate && (
          <span className="font-mono text-xs text-ink-dim/70 line-through decoration-2 decoration-ink-dim/70 pl-1 md:pl-0 md:pr-1">
            {item.oldDate}
          </span>
        )}
        <span className="inline-flex items-center gap-2 font-mono text-sm md:text-base font-bold text-ink bg-chrome-200 px-4 py-2 border border-ink shadow-[3px_3px_0px_0px_var(--color-ink)] md:-rotate-1 md:group-hover:rotate-[1.5deg] transition-transform duration-300">
